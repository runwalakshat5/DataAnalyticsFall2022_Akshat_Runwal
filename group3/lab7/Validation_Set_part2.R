...
# We use the subset option in the Im() function to fit a linear regression using,
...
mean((mpg-predict(Im.fit2,Auto))[-train]*2)

# Cubic regression line

lm.fit3 <- lm(mpg~poly(horsepower,3), data = Auto, subset = train) # Cubic
mean((mpg-predict(Im.fit3,Auto))[-train]*2)
...
# we will store the CV errors corresponding to the polynomial fits of orders one to #
# here the K =10
# Read the cv.gim documentation
set.seed(17)
